...
财采是**数据下载与整理**程序，不绑定单一回测引擎；时区统一北京，支持复权与多粒度。Parquet 为默认主存储格式（ZSTD 压缩），tickterial 采用 CSV 主产物并可桥接为 Parquet。
...
```text
数据源层 (httpx+YFinance / baostock+akshare / tickterial)
        ↓
QuantDB SQLite 缓存层 (cache hit <18ms)
        ↓
...
解析与校验 (Polars/日志/失败记录)
        ↓
存储 (data_root/asset_type/symbol/*.parquet，ZSTD 压缩)
        ↓
健康检查（doctor）→ 配置/目录/依赖/网络/scope 自检
        ↓
可选导出 (VectorBT npz / MT4 CSV)
```

- **数据源层**：`DataSource` 抽象（列出标的、拉取 OHLCV），实现包括：
  - `HttpxDataSource`：美股/外汇/加密，基于 httpx 异步客户端 + QuantDB 缓存
  - `YFinanceDataSource`：美股备用 fallback
  - `BaostockDataSource`：A 股，baostock 优先，akshare 自动降级
  - `TickterialDataSource`：Dukascopy 互补品种
  - `QuantDBDataSource`：包装 HttpxDataSource 的 SQLite 缓存层
...
- **存储**：`{data_root}/{asset_type}/{symbol}/{interval}_{start}_{end}.parquet`，ZSTD 压缩。
...
4. `writer`：合并区间并写入单文件（ZSTD Parquet）；
5. `validate`：扫描 parquet 质量告警；
6. `export`：转换为回测引擎输入（VectorBT npz 或 MT4 CSV）。
...
| `datagrab.sources.httpx_source` | 美股等 httpx 异步实现，含 QuantDB 缓存和 yfinance fallback |
| `datagrab.sources.yfinance_source` | 美股 yfinance 实现，含限速与回退 |
| `datagrab.sources.baostock_source` | A 股 baostock 实现，akshare 自动降级 |
| `datagrab.sources.quantdb_source` | QuantDB SQLite 缓存层，包装 httpx_source |
...
| `datagrab.tickterial.fetch_dukas` | dukascopy-python 后端适配器（主推） |
...
| `datagrab.tickterial.runner` | tickterial CLI 编排、CSV I/O、失败窗口与日志、Dukascopy 三级回退 |
...
| `datagrab.storage.export` | VectorBT npz / MT4 CSV 导出 |
| `datagrab.rate_limiter` | Token Bucket + Sliding Window 限流（突发容量 + 精确计数） |
...
<data_root>/
  ...
  ashare/
    sh.600000/
      1d_20200101_20241231.parquet
  .quantdb_cache/          # QuantDB SQLite 缓存
...
| asset_type | 数据源 | 说明 |
|---|---|---|
| stock | httpx+YFinance（QuantDB 缓存） | 美股等，目录来自 NASDAQ/otherlisted |
| ashare | baostock → akshare fallback | A 股，目录与复权由 baostock 提供 |
| forex / crypto / commodity | httpx+YFinance（QuantDB 缓存） | 下载走 httpx |
| tickterial（可选） | tickvault → dukascopy-python → tickterial | `--source tickterial` 拉取 Dukascopy 原始 tick，产出 CSV 后可 bridge 到 Parquet |
